...
class PostLikeListenerTest {
    ...
    @BeforeEach
    void setUp() {
        event = PostLikeEvent.builder().userId(String.valueOf(123L)).postLikeId(String.valueOf(456L)).build();
    ...
    void onMessage() throws IOException {
        ...
        when(objectMapper.readValue(body, PostLikeEvent.class)).thenReturn(event);
        when(postLikeMapper.toAnalytic(event)).thenReturn(analytic);
        listener.onMessage(message, pattern);
        InOrder inOrder = inOrder(analyticService, postLikeMapper);
        inOrder.verify(postLikeMapper).toAnalytic(event);
